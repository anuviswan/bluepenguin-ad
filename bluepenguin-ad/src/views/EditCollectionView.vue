...
const fetchCollection = async () => {
  ...
  try {
    ...
    if (collection) {
      ...
      status.value = collection.isActive ? 'active' : 'draft';
    } else {
...
const handleSave = async () => {
  ...
  try {
    await CollectionService.update(collectionId.value, collectionName.value.trim());
    successMessage.value = `Collection '${collectionName.value.trim()}' has been updated successfully! Redirecting...`;
    
    setTimeout(() => {
      router.push('/collections');
    }, 2000);
  } catch (err) {
    error.value = 'Failed to update collection. Please try again.';
    console.error(err);
  ...
  <MainLayout>
    <div class="edit-collection-page">
      ...
      <nav class="breadcrumbs flex align-center gap-2">
        <router-link to="/collections" class="breadcrumb-item">Collections</router-link>
        <span class="material-icons-outlined sep">chevron_right</span>
        <span class="breadcrumb-item active">Edit Collection</span>
      </nav>
      ...
      <div v-if="isLoading" class="loading-state card p-8 flex justify-center align-center">
        ...
        <span class="ml-2">Loading collection details...</span>
      </div>
      ...
      <div v-else class="form-container card">
        <div class="form-section">
          ...
          <input 
            ...
            disabled
          />
          <small class="text-muted">Collection ID cannot be changed.</small>
        </div>
